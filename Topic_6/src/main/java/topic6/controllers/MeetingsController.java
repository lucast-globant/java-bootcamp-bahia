...
public class MeetingsController {
	...
	public ResponseEntity<Meeting> createMeeting(@RequestBody Meeting meeting) {
		Meeting m = meetings.save(meeting);
	...
	public ResponseEntity<Meeting> readMeeting(@PathVariable Long id) {
		Meeting m = meetings.findOne(id);
		if(m != null) return new ResponseEntity<Meeting>(m,HttpStatus.OK);
		else return new ResponseEntity<Meeting>(HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<Meeting> updateMeeting(@RequestBody Meeting meeting){
		if(meetings.exists(meeting.getId())){
			Meeting m = meetings.saveAndFlush(meeting);
